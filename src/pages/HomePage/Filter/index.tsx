...
export const Filter = ({ setFilter, setAdvertisements, advertisements }: any) => {
  ...
  useEffect(() => {
    ...
    if (urlBrand) {
      advertisements.map((car: { model: string, color: string, year: string, fuel: string }) => {
          ...
          <li>
            ...
            <ul className="lists">
              {models?.map((model: any, index: number) => (
                <li onClick={() => setModelFunction(model, index)} key={index}>{model}</li>
              ))}
            </ul>
          </li>
          <li className="listModels">
            ...
            <ul className="lists listModels">
              {colors.map((color, index: number) => (
                <li id={color} onClick={() => setColorFunction(color)} key={index}>{color}</li>
              ))}
            </ul>
          </li>
          <li>
            ...
            <ul className="lists">
              {years.map((year: string, index: number) => (
                <li onClick={() => setYearFunction(year)} id={year} key={index}>{year}</li>
              ))}
            </ul>
          </li>
          <li>
            ...
            <ul className="lists">
              {fuel.map((fuel: string, index: number) => (
                <li onClick={() => setFueFunction(fuel)} key={index}>{fuel}</li>
              ))}
            </ul>
          </li>
        </ul>
        <form onKeyDown={(e) => eventKey(e)}>
          <div className="range">
            ...
            <div className="inputs">
              ...
              <input onChange={(e) => setKmMin(e.target.value)} type="number" placeholder="Mínima" />
              <input onChange={(e) => setKmMax(e.target.value)} type="number" placeholder="Máxima" />
            </div>
            <div className="range">
              <h3>Year</h3>
              <div className="inputs">
                <input onChange={(e) => setYearMin(e.target.value)} type="number" placeholder="Mínimo" />
                <input onChange={(e) => setYearMax(e.target.value)} type="number" placeholder="Máximo" />
              </div>
            </div>
          </div>
        </form>
      </div>
      {urlBrand || kmMin || kmMax || yearMin || yearMax ? (<button onClick={() => cleanFilter()} >Limpar Filtro</button>) : (<></>)}
